...
class EbayFranceBikeSpider(CrawlSpider):
    name = 'ebay_france_bike'
    start_urls = ['https://www.ebay.fr/sch/177831/i.html?_from=R40&_nkw=giant+v%C3%A9los&LH_TitleDesc=0&_sop=10&_ipg=60']
    ...
    rules = (
        Rule(LinkExtractor(restrict_css='.pagination__next', deny=['_pgn=1','_pgn=3'])),
    ...
    def parse_element_detail(self, response):          # Function to extract data from fetched urls
        ...
        key_value_lang = ["État:", "Année De Fabrication:", "Shifter Style:", "Lenkerart:", "Nombre de vitesses:", "Couleur:", "Wheel Size:", "Federung:", "type de vélo:", "Marque:",
                          "Taille du cadre:", "Matériau:", "Département:", "Modèle:", "Type de frein:", "Reifentyp:", "MPN:", "Vintage", "Gear Change Mechanism", "UPC", "Frame Number",
                          "Features", "Poids Du Produit:", "Stem", "Saddle", "Drivetrain Type", "Seatpost", "Genre:", "Crankset", "Chainrings", "Cassette", "Wheel Set",
        ...
        for i in range(len(specification_u)-1):
            for (j, k) in zip(key_value, key_value_lang):
                ...
                try:
                    if "Model Year" in specification_u[i] or "Année De Fabrication:" in specification_u[i]:
        ...
        if len(response.css("#prcIsum::text").extract()) != 0:
            ...
            item["price"] = str(response.css("#prcIsum::text").extract()).strip("['").strip("']").replace("\\xa0","")
            item["starting_bid"] = ""
            item["total_bid"] = ""

        elif len(response.css("#mm-saleDscPrc::text").extract()) != 0:

            item["price"] = str(response.css("#mm-saleDscPrc::text").extract()).strip("['").strip("']").replace("\\xa0","")
            item["starting_bid"] = ""
            item["total_bid"] = ""
