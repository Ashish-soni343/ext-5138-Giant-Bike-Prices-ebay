...
class EbayGermanyBikeSpider(CrawlSpider):
    name = 'ebay_germany_bike'
    start_urls = ["https://www.ebay.de/sch/i.html?_from=R40&_nkw=giant+Fahrr%C3%A4der&_sacat=0&_sop=10&_pgn=1"]
    ...
    def parse_element_detail(self, response):          # Function to extract data from fetched urls
        ...
        key_value_lang = ["Artikelzustand:", "Jahr:", "Shifter Style:", "Lenkerart:", "Number of Speeds:", "Farbe:", "Wheel Size:", "Federung:", "Fahrradart:", "Marke:",
                          "Reifengröße:", "Cadre:", "Abteilung:", "Modell:", "Bremsmethode:", "Reifentyp:", "MPN:", "Vintage", "Gear Change Mechanism", "UPC", "Frame Number",
        ...
        for i in range(len(specification_u)-1):
            for (j, k) in zip(key_value, key_value_lang):
                ...
                if "Taille de roue:" in specification_u[i]:
                    item["Wheel_Size"] = specification_u[i + 1]
                if "Nombre de vitesses:" in specification_u[i]:
                    item["Number_of_Speeds"] = specification_u[i + 1]
                if "Fahrrad:" in specification_u[i]:
                    item["Bike_Type"] = specification_u[i + 1]
                if "Genre" in specification_u[i]:
                    item["Gender"] = specification_u[i + 1]
                if "Marca:" in specification_u[i] or "Marque:" in specification_u[i] or "Marka:" in specification_u[i] or "Merk:" in specification_u[i]:
                    item["Brand"] = specification_u[i + 1]
                if "Tipo:" in specification_u[i]:
                    item["Bike_Type"] = specification_u[i + 1]
