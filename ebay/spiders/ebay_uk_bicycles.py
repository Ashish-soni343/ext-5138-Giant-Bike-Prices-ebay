...
class EbayUKBicyclesSpider(CrawlSpider):
    name = 'ebay_uk_bicycles'
    start_urls = ["https://www.ebay.co.uk/sch/i.html?_from=R40&_nkw=Giant+bicycles&_sacat=0&_sop=10&_ipg=240"]
    ...
    def parse_element_detail(self, response):          # Function to extract data from fetched urls
        ...
        for i in range(len(specification_u)-1):
            t = 0
            for j in key_value:
                if ":" in specification_u[i + 1] and i >= 3 and specification_u[i] == "See all condition definitions":
                    break
