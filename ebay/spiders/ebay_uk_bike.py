...
class EbayUkBikeSpider(CrawlSpider):
    name = 'ebay_uk_bike'
    start_urls = ["https://www.ebay.co.uk/sch/i.html?_from=R40&_nkw=Giant+bike&_sacat=0&_ipg=240&_sop=10&_pgn=1"]
    ...
    rules = (
        Rule(LinkExtractor(restrict_css='.pagination__next', deny=['_pgn=1','rt=nc'])),
    ...
    def parse_element_detail(self, response):          # Function to extract data from fetched urls
        ...
        Breads = response.css('li a.seo-breadcrumb-text span::text').extract()
        if len(Breads) != 0:
        ...
        key_value_lang = ["Condizione:", "Jahr:", "Shifter Style:", "Lenkerart:", "Numero di cambi",
                          "Colore principale:", "Wheel Size:", "Federung:", "Tipo:", "Marke:", "Dimensioni ruota:", "Materiale telaio:", "Abteilung:", "Modell:", "Tipo di freno:",
                          "Reifentyp:", "MPN:", "Vintage", "Gear Change Mechanism", "UPC", "Frame Number",
                          "Features", "Item Weight", "Stem", "Saddle", "Drivetrain Type", "Seatpost", "Genere:",
        ...
        for i in range(len(specification_u)-1):
            for (j, k) in zip(key_value, key_value_lang):
                ...
                if "Tyre type" in specification_u[i]:
                    item["Tire_Type"] = specification_u[i + 1]
                if "Used" in specification_u[i] and len(specification_u[i+1]) == 0:
                    item["Condition"] = "Used"
                if "Marca:" in specification_u[i]:
