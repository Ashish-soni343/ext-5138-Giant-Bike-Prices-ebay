...
class EbayIrelandBikeSpider(CrawlSpider):
    name = 'ebay_ireland_bike'
    start_urls = ["https://www.ebay.ie/sch/i.html?_from=R40&_nkw=giant+bicycles&_sacat=0&LH_TitleDesc=0&_sop=10&_ipg=60&_pgn=1"]

    # Mandatory data
    #AEID_project_id = ''
    type = "Bike"
    site = 'https://www.ebay.ie'
    ...
    def parse_element_detail(self, response):          # Function to extract data from fetched urls
        ...
        for i in range(len(specification_u)-1):
            for j in key_value:
                ...
                if j in specification_u[i] and specification_u[i+1] != "See all condition definitions":
